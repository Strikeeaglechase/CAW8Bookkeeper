...
class EditRemarks extends SlashCommand {
    name = "remarks";
    description = "Updates a users remarks";
    async run({ interaction, app, framework }, name, remarks) {
        ...
        if (requireConfirmation) {
            const conf = await interactionConfirm(`Are you sure you want to update ${name}'s remarks from "${existingUserEntry.remarks}" to "${remarks}"?`, interaction);
            if (!conf) {
                return;
            }
        }
        await app.ops.collection.updateOne({ id: op.id }, { $set: { "members.$[elem].remarks": remarks } }, { arrayFilters: [{ "elem.name": name }] });
        await interaction.editReply(framework.success(`${name} changed from "${existingUserEntry.remarks}" to "${remarks}"`));
    }
