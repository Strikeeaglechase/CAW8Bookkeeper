...
class EditCombatDeaths extends SlashCommand {
    ...
    async run({ interaction, app, framework }, name, combatDeaths) {
        ...
        if (requireConfirmation) {
            const conf = await interactionConfirm(`Are you sure you want to update ${name}'s deaths from "${existingUserEntry.combatDeaths}" to "${combatDeaths}"?`, interaction);
            if (!conf) {
                return;
            }
        }
...
], EditCombatDeaths.prototype, "run", null);
export default EditCombatDeaths;
//# sourceMappingURL=editCombatDeaths.js.map