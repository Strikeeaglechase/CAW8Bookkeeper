...
class Application {
    ...
    async loadOldOp(op) {
        ...
        if (!existing)
            await this.ops.add(dbOp);
    }
    async importOp(sheetName) {
        ...
        if (!targetSheet)
            return `Sheet \`${sheetName}\` was not found`;
        await targetSheet.loadCells();
        const opName = targetSheet.getCell(0, 0).value;
        const oOps = [];
        for (let startCell = 1; startCell < 115; startCell += 19) {
            const timeslot = targetSheet.getCell(startCell, 0).value;
            const oldOpEntry = {
                name: opName,
                timeslot: timeslot,
                members: []
            ...
            for (let i = 0; i < 16; i++) {
                const name = targetSheet.getCell(startCell + 2 + i, 0).value;
                const callsign = targetSheet.getCell(startCell + 2 + i, 1).value;
                const type = targetSheet.getCell(startCell + 2 + i, 2).value;
                const bolters = targetSheet.getCell(startCell + 2 + i, 3).value;
                const wire = targetSheet.getCell(startCell + 2 + i, 4).value;
                const lsoGrade = targetSheet.getCell(startCell + 2 + i, 6).value;
                const combatDeaths = targetSheet.getCell(startCell + 2 + i, 7).value;
                const promotions = targetSheet.getCell(startCell + 2 + i, 8).value;
                const remarks = targetSheet.getCell(startCell + 2 + i, 9).value;
                if (!name)
                    continue;
            ...
            if (oldOpEntry.members.length > 0)
                oOps.push(oldOpEntry);
        }
